...
// AuthSrvHttp represents a client that can talk to the auth service.
type AuthSrvHttp struct {
	url     string
	logFunc func(ctx context.Context, s string)
	client  *http.Client
}

// NewHttp constructs an Auth that can be used to talk with the auth service.
func NewHttp(url string, logFunc func(ctx context.Context, s string), options ...func(authSrv *AuthSrvHttp)) *AuthSrvHttp {
	authSrvHttp := AuthSrvHttp{
		url:     url,
		logFunc: logFunc,
...
func WithClient(client *http.Client) func(authAPI *AuthSrvHttp) {
	return func(authSrv *AuthSrvHttp) {
		authSrv.client = client
	}
}

// Authenticate calls the auth service to authenticate the user.
func (srv *AuthSrvHttp) Authenticate(ctx context.Context, authorization string) (AuthenticateResp, error) {
...
func (srv *AuthSrvHttp) Authorize(ctx context.Context, auth Authorize) error {
...
func (srv *AuthSrvHttp) rawRequest(ctx context.Context, method string, url string, headers map[string]string, r io.Reader, v any) error {
